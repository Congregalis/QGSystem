...
public interface ContextRepository extends JpaRepository<Context, Long> {
    ...
    @Query(value = "select distinct context.* from context inner join question  on context.id=question.contextId where if(?1 !='',language = ?1,1=1)  and if(?2 !='',title = ?2,1=1) and if(?3 !='',subject = ?3,1=1) and if(?4 !='',origin = ?4,1=1) and if(?5 !='',questionType = ?5,1=1) and if(?6 !='',whType = ?6,1=1) and if(?7 !='',cognitiveType = ?7,1=1) and if(IFNULL(?8,'')!='',fluency = ?8,1=1) and if(IFNULL(?9,'')!='',relevance = ?9,1=1) and if(IFNULL(?10,'')!='',difficulty = ?10,1=1) and if(IFNULL(?11,'')!='',reasonable = ?11,1=1) and if(IFNULL(?12,'')!='',score = ?12,1=1) and contextIsDeleted=0  order by context.id ;", nativeQuery = true)
